[CmdletBinding()] 
param([string]$InputFile=".\config.xml",[Switch]$Dev)
...
try {
	Start-Transcript "$thisscript.log"
} catch {}
...
Write-verbose "$(Get-Date -Format "HH:mm:ss") : Importing GPOToolLib.ps1"
. .\GPOToollib.ps1
LoadHTMLDiff
...
$domains=@()
$domains+= GetTrustedDomains $config
$domains+= GetLocalDomain $config
$cssstyle=Get-CSS
...
foreach ($domain in ($Domains|?{[System.Convert]::ToBoolean($_.Backup)})) {
...
Write-Verbose "Creating OUDiff subfolder if it does not exist"
$temppath=[io.path]::Combine($config.Export.path,"OUDiff")
New-Item -ItemType Directory -Force -Path $temppath
$outfile=[io.path]::combine($temppath,"OUDiff_$runtime.htm")
$testhtml | Out-File $outfile
if ($dev) {
	ii $outfile
} else {
	if ((Get-Date).dayofweek -eq "Sunday") {
		send-SMTPmail -to $config.Mail.Recipient -from $config.Mail.Sender -attachment $outfile -subject "EET AD Police - OUs Report" -smtpserver $config.Mail.SmtpHost
	}
}
...
$disabledGPOs=@()
$emptyGPOs=@()
foreach ($domain in ($Domains|?{[System.Convert]::ToBoolean($_.Report)})) {
...
Write-Verbose "Creating ObsoletedGPOs subfolder if it does not exist"
$temppath=[io.path]::Combine($config.Export.path,"ObsoletedGPOs")
New-Item -ItemType Directory -Force -Path $temppath
$outfile=[io.path]::combine($temppath,"ObsoletedGPOs_$runtime.htm")
$html| Out-File $outfile
if ($dev) {
	ii $outfile
} else {
	if ((Get-Date).dayofweek -eq "Sunday") {
		send-SMTPmail -to $config.Mail.Recipient -from $config.Mail.Sender -attachment $outfile -subject "EET AD Police - Obsolete GPOs Report" -smtpserver $config.Mail.SmtpHost
	}
}
...
Write-Verbose "Creating GPDiff subfolder if it does not exist"
$temppath=[io.path]::combine($config.Export.path,"GPDiff")
New-Item -ItemType Directory -Force -Path $temppath
$outfile=[io.path]::combine($temppath,"GPDiff_$runtime.htm")
$testhtml | Out-File $outfile
if ($dev) {
	ii $outfile
	#send-SMTPmail -to $config.Mail.Recipient -from $config.Mail.Sender -attachment $outfile -subject "EET AD Police - GPOs Report" -smtpserver $config.Mail.SmtpHost
} else {
	if ((Get-Date).dayofweek -eq "Sunday") {
		send-SMTPmail -to $config.Mail.Recipient -from $config.Mail.Sender -attachment $outfile -subject "EET AD Police - GPOs Report" -smtpserver $config.Mail.SmtpHost
	}
...
Write-Verbose "Creating GPLinkDiff subfolder if it does not exist"
$temppath=[io.path]::combine($config.Export.path,"GPLinkDiff")
New-Item -ItemType Directory -Force -Path $temppath
$outfile=[io.path]::combine($temppath,"GPLinkDiff_$runtime.htm")
$report | Out-File $outfile
#$AllNeutralGPOLinks | ConvertTo-Html | Out-File $outfile
if ($dev) {
	ii $outfile
} else {
	if ((Get-Date).dayofweek -eq "Sunday") {
		send-SMTPmail -to $config.Mail.Recipient -from $config.Mail.Sender -attachment $outfile -subject "EET AD Police - GPO Link Difference Report" -smtpserver $config.Mail.SmtpHost
	}
}
